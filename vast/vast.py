...
class Vast(object):
    ...
    async def get_future(self, fn: Callable, args: list= [], kwargs: dict= {}):
        return self.loop.run_in_executor(self.executor, partial(self._execute, fn, args, kwargs))
    
    async def run_in_executor(self, fn: Callable, listOfArgs: List[Tuple[list, dict]]):
        with ThreadPoolExecutor(max_workers= self.workers) as self.executor:
            return [
                await asyncio.gather(
                    *[self.get_future(fn, *args) for args in listOfArgs[index:index+self.workers]]
                )
    ...
    def run_in_eventloop(self, fn: Callable, listOfArgs: List[Tuple[list, dict]]= list((list, dict))):
        ...
        return [future.result() for future_results in self.loop.run_until_complete(self.run_in_executor(fn, listOfArgs)) for future in future_results]
